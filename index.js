...
const start = (kill = new Client()) => {
    console.log(color('\n[DEV]', 'red'), color('- Ruan P <-> +55 31 99689-1466 '))
	console.log(color('[alexa]', 'red'), color('Minha inicialização foi concluída, você pode usar agora...\n'))
	
		// Forçar recarregamento caso obtenha erros
		kill.onStateChanged((state) => {
			console.log('[Estado da alexa]', state)
		...
		kill.onGlobalParticipantsChanged(async (event) => {
			...
			try {
				if (event.action == 'add') {
					if (isAnti && fuck) {
						await kill.sendText(event.chat, `Bem vindo dnv`)
						await sleep(2000)
						...
						const welcomer = await new canvas.Welcome()
							...
							.setBackground('https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcT8uEbriPMTt-usA4dcv14HMXNrJfV54VLzYg&usqp=CAU')
							.toAttachment()
					...
					await kill.sendFile(event.chat, base64, 'welcome.png', `Vix, mais um membro se foi, sentiremos sua falta`)
