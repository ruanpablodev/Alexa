...
const { owner, donate, down, rpg, animesa, help, admins, adult, readme, lang, convh, paises, avan, animais, estudo, logo, painel, stickerss, outros, letras } = require('./lib/help2')
...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
        ...
        if (isGroupMsg && autoSticker && isMedia && isImage && !isCmd) {
            ...
            await kill.sendImageAsSticker(from, imageBase64, { author: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', pack: ' ALEXA BOT ⚜', keepScale: true })
        }
        ...
        if (isGroupMsg && autoSticker && isMedia && isVideo && !isCmd) {
            ...
            await kill.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', author: '  ALEXA BOT ⚜', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
        }
        ...
        case 'stiker':
            if (isMedia && isImage) {
				const mediaData = await decryptMedia(message, uaOverride)
				const imageBase64 = `data:${mimetype};base64,${mediaData.toString('base64')}`
				await kill.sendImageAsSticker(from, imageBase64, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
                ...
                const url = args[0]
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
		...
		case 'ttp':
			...
			.then(res => {
				kill.sendImageAsSticker(from, res.data.result, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
			})
			break
        ...
        case 'stickernobg':
			if (isMedia) {
                try {
                    ...
                    await kill.sendImageAsSticker(from, `data:${mimetype};base64,${result.base64img}`, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
        ...
        case 'gif':
            if (isMedia && type === 'video' || mimetype === 'image/gif' || isQuotedVideo || isQuotedGif) {
                ...
                try {
                    ...
                    await kill.sendMp4AsSticker(from, gifSticker, null, { stickerMetadata: true, pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', author: ' ALEXA BOT ⚜', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: false, loop: 0 })
                } catch (err) {
                    console.error(err)
        ...
        case 'makesticker':
			...
			console.log(stkfm)
            await kill.sendStickerfromUrl(from, stkfm, { method: 'get' }, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
			.catch(() => {
        ...
        case 'detector':
			...
			await kill.reply(from, 'Calculando foto dos participantes do grupo...', id)
            await sleep(3000)
            const eu2 = await kill.getGroupMembers(groupId)
            const sortear = eu2[Math.floor(Math.random() * eu2.length)]
			console.log(sortear.id)
            await kill.sendTextWithMentions(from, `O ganhador do sorteio foi o @${sortear.id.replace(/@c.us/g, '')}`)
            await sleep(2000)
            break	
       ...
       case 'translate':
		...
		case 'resposta':
			...
			await kill.reply(from, 'Frase adicionada a alexa.', id)
			break
        ...
        case 'alexa':
			...
			try {
				...
				if (alexa.data.success == '' || alexa.data.success == 'Limit 50 queries per hour.') {
        ...
        case 'roll':
            ...
            await kill.sendStickerfromUrl(from, 'https://www.random.org/dice/dice' + dice + '.png', { method: 'get' }, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
            break
        ...
        case 'flip':
            ...
            if (side == 1) {
               kill.sendStickerfromUrl(from, 'https://i.ibb.co/LJjkVK5/heads.png', { method: 'get' }, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
            } else {
               kill.sendStickerfromUrl(from, 'https://i.ibb.co/wNnZ4QD/tails.png', { method: 'get' }, { author: ' ALEXA BOT ⚜', pack: 'FIGURINHA FEITO PELO BOT DO PATO, ALEXA BOT ☆', keepScale: true })
            }
            break
        ...
        case 'bemvindo':
			...
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				...
				if (args[0] == 'on') {
					...
					kill.reply(from, 'Feito! As funções de Boas-Vindas e Adeus foram acionadas.', id)
				} else if (args[0] == 'off') {
					let welcom = welkom.indexOf(chatId)
					welkom.splice(welcom, 1)
					fs.writeFileSync('./lib/config/welcome.json', JSON.stringify(welkom))
					kill.reply(from, 'Entendido! Desativei as opções de Boas-Vindas e Adeus.', id)
				} else {
					kill.reply(from, 'Você esqueceu de colocar se quer ativado [on], ou desativado [off].', id)
				}
			} else if (isGroupMsg) {
       ...
       case 'clima':
        ...
        case 'broad':
			...
			if (args[0] == '-all') {
				...
				for (let ids of chatz) {
					...
					if (!cvk.isReadOnly) {
						await kill.sendText(ids, `[Transmissão do dono da alexa]\n\n${msg}`)
					} else {
				...
				for (let bclst of chatz) {
					...
					if (!notgps) {
						...
						if (!bkgps.isReadOnly) {
							await kill.sendText(bclst, `[Transmissão do dono da alexa]\n\n${msg}`)
						} else {
							console.log("Ignorei um grupo/privado pois estava fechado.")
						}
					} else return
        ...
        case 'encerrar':
            if (!isOwner) return kill.reply(from, 'Somente o meu criador tem acesso a este comando.', id)
			if (!isOwner) return kill.reply(from, mess.error.Kl, id)
			const chatz2 = await kill.getAllChatIds()
			if (args[0] == '-all') {
				for (let ids of chatz2) {
					var cvk = await kill.getChatById(ids)
					if (!cvk.isReadOnly) {
						await kill.sendText(ids, `IREI FICAR OFF EM  SEGUNDOS`)
					} else {
						console.log("Ignorei um grupo/privado pois estava fechado.")
					}
				}}
		...
		case 'avançado':
			case 'avancado':
            kill.reply(from, avan, id)
            break
		case 'animais':
				kill.reply(from, animais, id)
			break

			case 'estudo':
				kill.reply(from, estudo, id)
			break
			case 'logo':
				kill.reply(from, logo, id)
			break

			case 'painel':
				kill.reply(from, painel, id)
			break

			case 'stickers':
				kill.reply(from, stickerss, id)
			break

			case 'outros':
				kill.reply(from, outros, id)
			break

			case 'letras':
				kill.reply(from, letras, id)
			break
		...
		case 'help':
			if (args.length == 0) return kill.reply(from, 'Defina seu problema a ser enviado ao grupo responsável pela alexa.', id)
        ...
        case 'level':
            ...
            const ranq = new canvas.Rank()
                .setAvatar(pepe)
                .setLevel(userLevel)
                .setLevelColor('#04b52a', '#04b52a')
                .setRank(Number(rank.getRank(usuario, nivel)))
                .setCurrentXP(userXp)
                .setOverlay('#000000', 100, false)
                .setRequiredXP(requiredXp)
                .setProgressBar('#04b52a', 'COLOR')
		...
		case 'email':
			...
			try {
				...
				const mails = await axios.get(`https://videfikri.com/api/spamemail/?email=${emailsd}&subjek=${assuml}&pesan=${textoma}%20%20%20%20Email%20enviado%20pelo%20bot%20de%20whatsapp%20AlexaPremium`)
